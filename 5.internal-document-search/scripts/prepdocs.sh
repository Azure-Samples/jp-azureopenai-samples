...
# if you want to use a different python script, set the path here
pyscriptPath="./scripts/prepdocs_v2.py"
...
storageKey=$(az storage account keys list -g "$AZURE_COSMOSDB_RESOURCE_GROUP" -n "$AZURE_STORAGE_ACCOUNT" | grep -Po '"value": "\K[^"]*' | head -n1)
...
./scripts/.venv/bin/python $pyscriptPath './data/*' --storageaccount "$AZURE_STORAGE_ACCOUNT" --container "$AZURE_STORAGE_CONTAINER" --storagekey "$storageKey" --searchservice "$AZURE_SEARCH_SERVICE" --index "$AZURE_SEARCH_INDEX" --formrecognizerservice "$AZURE_FORMRECOGNIZER_SERVICE" --tenantid "$AZURE_TENANT_ID" -v
