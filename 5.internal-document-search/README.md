...
### 検索対象のPDFファイルの更新
このサンプルでは、`data/`ディレクトリにある PDF ファイルを Azure Cognitive Search にインデックスを作成するために使用しています。検索可能な PDF ファイルを変更する場合には、以下の手順を実行してください。

1. `data/`ディレクトリに新しい PDF ファイルを追加する。
